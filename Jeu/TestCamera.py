...
def DrawPlayer(entity,u,v):                                               #Fonction permettant d'afficher une entité de 32 pixels et 4 frame d'animations.
    if entity['Sens']=='Droite':                                                    #Regarde si il regarde à Gauche
        if entity['Frame']==0:                                                           #Les différentes Frames d'animations
            pyxel.blt(Xmap-16,Ymap-16,1,u,v,32,32,1)
        elif entity['Frame']==1:
            pyxel.blt(Xmap-16,Ymap-16,1,u,v+32,32,32,1)
        elif entity['Frame']==2:
            pyxel.blt(Xmap-16,Ymap-16,1,u,v+64,32,32,1)
        else:
            pyxel.blt(Xmap-16,Ymap-16,1,u,v+96,32,32,1)
    if entity['Sens']=='Gauche':                                                    #Regarde si il regarde à Droite
        if entity['Frame']==0:                                                            #Les différentes Frames d'animations
            pyxel.blt(Xmap-16,Ymap-16,1,u,v,-32,32,1)
        elif entity['Frame']==1:
...
'''
Player=dict()
Player['y']=0                                                                 #Indique l'emplacement Y du Joueur
Player['x']=0                                                                 #Indique l'emplacement X du Joueur
...
def deplacement():                                                              #Déplacement Du joueur                                                                                                                                   #Déplacement Vers le bas 
    if pyxel.btn(pyxel.KEY_DOWN) or pyxel.btn(pyxel.KEY_S):
        Player['y']=Player['y']-Player['Vitesse']
        Bot2['y']=Bot2['y']-Player['Vitesse']
        Bot1['y']=Bot1['y']-Player['Vitesse']                                   #Déplacement Vers la Droite et modifie le sens vers la droite    
    if pyxel.btn(pyxel.KEY_RIGHT) or pyxel.btn(pyxel.KEY_D):
        Player['x']=Player['x']-Player['Vitesse']
        Bot2['x']=Bot2['x']-Player['Vitesse']
        Bot1['x']=Bot1['x']-Player['Vitesse']
        Player['Sens']='Droite'                                                       #Déplacement Vers la gauche et modifie le sens vers la gauche 
    if pyxel.btn(pyxel.KEY_LEFT) or pyxel.btn(pyxel.KEY_Q):
        Player['x']=Player['x']+Player['Vitesse']
        Bot2['x']=Bot2['x']+Player['Vitesse']
        Bot1['x']=Bot1['x']+Player['Vitesse']
        Player['Sens']='Gauche'                                                         #Déplacement Vers le haut 
    if pyxel.btn(pyxel.KEY_UP) or pyxel.btn(pyxel.KEY_Z):
        Player['y']=Player['y']+Player['Vitesse']
        Bot2['y']=Bot2['y']+Player['Vitesse']
        Bot1['y']=Bot1['y']+Player['Vitesse']
...
def Batterie():                                                                 #Diminue la luminosité de la lampe torche en fonction de la Batterie  
    ...
    if Player['Batterie']<=100:
        ...
        pyxel.blt(Xmap-80,Ymap-80,2,64,0,32,16,1)    
    if Player['Batterie']<=80:
        Player['Vue']=55
        pyxel.blt(Xmap-80,Ymap-80,2,64,16,32,16,1)
    if Player['Batterie']<=60:
        Player['Vue']=45
        pyxel.blt(Xmap-80,Ymap-80,2,64,32,32,16,1)
    if Player['Batterie']<=40:
        Player['Vue']=35
        pyxel.blt(Xmap-80,Ymap-80,2,64,48,32,16,1)
    if Player['Batterie']<=20:
        Player['Vue']=20
        pyxel.blt(Xmap-80,Ymap-80,2,64,64,32,16,1)
...
def Lampe():                                                                    #Affiche une partie de la map autour du joueur en fonction de la visibilité 
    for x in range(Xmap-75,Xmap+75):
        for y in range(Ymap-75,Ymap+75):
            b=pyxel.sqrt((x-Xmap)**2+(y-Ymap)**2)
            if b>Player['Vue']:
                pyxel.pset(x,y,0)
...
def LampeLum():
    for x in range(Xmap-66,Xmap+66):
        for y in range(Ymap-66,Ymap+66):
            b=pyxel.sqrt((x-Xmap)**2+(y-Ymap)**2)
            if b<Player['Vue']-22:
                pyxel.pset(x,y,10)
...
class App:
    ...
    def draw(self):
        ...
        if start==1:                                                            #Si la partie est démarrée
            pyxel.clip(320-75,160-75,150,150)                   
            pyxel.cls(0)                                                        #colorie toute la map en noir
            LampeLum()                                                          #affiche les couleurs de la lampe  
            pyxel.bltm(Player['x'],Player['y'],0,0,0,4080,4080,1)                                     #imprime la tilemap
            Lampe()                                                             #affiche qu'une certaine partie de la map a l'écran
                                                                                           
            DrawPlayer(Player,0,0)                                              #affiche le joueur et des animations
            
            
                                                                                #Si l'ennemis est dans le flash : il est visible
            if ((-Bot2['x']+Xmap) < Player['Vue'] and (Bot2['x']-Xmap) < Player['Vue']) and ((-Bot2['y']+Ymap) < Player['Vue'] and (Bot2['y']-Ymap) < Player['Vue'])  :
                Draw32px(Bot2,224,0,-1)
            
            
                                                                                #Si le bot regarde a droite : sprite a droite et inversement + Flash (comme au-dessus)
            if ((-Bot1['x']+Xmap) < Player['Vue'] and (Bot1['x']-Xmap) < Player['Vue']) and ((-Bot1['y']+Ymap) < Player['Vue'] and (Bot1['y']-Ymap) < Player['Vue'])  :
...
'''
            ...
            pyxel.text(Xmap-75,Ymap-75,XX,2)                                                #Position Y du curseur en haut a droite de l'écran                                   
            pyxel.text(Xmap-45,Ymap-75,YY,2)                                               #Position Y du curseur en haut a droite de l'écran 
'''